...
/**
 * Spring MVC interceptor (NOT a Servlet filter) that enforces {@link RequiresModule}.
 *
 * <p>Runs as a HandlerInterceptor — AFTER DispatcherServlet resolves the handler
 ...
 *
 * <p>Must be registered in a {@code WebMvcConfigurer#addInterceptors} call.
...
public class ModuleGuardInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(jakarta.servlet.http.HttpServletRequest request,
        ...
        if (moduleAnnotation != null && !isModuleAllowed(moduleAnnotation.value())) {
            throw new BusinessException(
                    ...
                    "Module '" + moduleAnnotation.value() + "' is not enabled for the active tenant."
            );
        }

        RequiresPermission permissionAnnotation = resolvePermissionAnnotation(method);
        if (permissionAnnotation != null && !RequestContext.hasPermission(permissionAnnotation.value())) {
            throw new BusinessException(
                    HttpStatus.FORBIDDEN,
                    ApiErrorCode.AUTH_PERMISSION_DENIED.value(),
                    "Permission '" + permissionAnnotation.value() + "' is not granted for the active user."
            );
        }
    ...
    private boolean isModuleAllowed(String moduleCode) {
        if (RequestContext.isSystemAdmin()) return true;
    ...
    private RequiresModule resolveModuleAnnotation(HandlerMethod method) {
        ...
        RequiresModule m = AnnotationUtils.findAnnotation(method.getMethod(), RequiresModule.class);
        return m != null ? m : AnnotationUtils.findAnnotation(method.getBeanType(), RequiresModule.class);
    }

    private RequiresPermission resolvePermissionAnnotation(HandlerMethod method) {
        // Method-level first, then class-level
        RequiresPermission p = AnnotationUtils.findAnnotation(method.getMethod(), RequiresPermission.class);
        return p != null ? p : AnnotationUtils.findAnnotation(method.getBeanType(), RequiresPermission.class);
    }
}
